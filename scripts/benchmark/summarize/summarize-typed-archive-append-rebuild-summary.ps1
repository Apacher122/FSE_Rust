...
for ($Index = 0; $Index -lt $Lines.Count; $Index++) {
  if ($Lines[$Index].Trim() -eq "Workload typed archive append rebuild timing summary") {
    $SectionIndex = $Index
    break
  }
}
...
if ($SectionIndex -lt 0) {
  throw "debug output does not contain a workload typed archive append rebuild timing summary section"
}
...
for ($Index = $SectionIndex + 1; $Index -lt $Lines.Count; $Index++) {
  ...
  if ($Trimmed.StartsWith("workload | base records | appended records | resulting records | before bytes | after bytes | byte growth | matched after append | append rebuild | agreement")) {
    $HeaderIndex = $Index
    break
  }
}
...
if ($HeaderIndex -lt 0) {
  throw "workload typed archive append rebuild timing summary header was not found"
}
...
for ($Index = $HeaderIndex + 1; $Index -lt $Lines.Count; $Index++) {
  ...
  if ($Columns.Count -ne 10) {
    throw "unexpected typed archive append rebuild timing summary column count at line $($Index + 1): $($Columns.Count)"
  }
...
if ($Rows.Count -eq 0) {
  throw "workload typed archive append rebuild timing summary contained no workload rows"
}
...
$TotalMatchedRecordsAfterAppend = 0

foreach ($Row in $Rows) {
  $TotalMatchedRecordsAfterAppend += Convert-ToInvariantIntOrZero -Value $Row[9]
}
...
$ResultingRecordCounts = @($Rows | ForEach-Object { $_[5] } | Sort-Object -Unique)
$ArchiveByteGrowthValues = @($Rows | ForEach-Object { $_[8] } | Sort-Object -Unique)

Set-Utf8Text -Path $OutputNotesPath -Text "Typed archive append rebuild timing summary notes`r`n"
...
Add-Utf8Text -Path $OutputNotesPath -Text "Total matched records after append: $TotalMatchedRecordsAfterAppend`r`n"
...
Add-Utf8Text -Path $OutputNotesPath -Text "Archive byte growth values: $($ArchiveByteGrowthValues -join ', ')`r`n"
...
Write-Host "Typed archive append rebuild timing summary written:"
