...
for ($Index = 0; $Index -lt $Lines.Count; $Index++) {
  if ($Lines[$Index].Trim() -eq "Workload typed archive maintenance timing summary") {
    $SectionIndex = $Index
    break
  }
}
...
if ($SectionIndex -lt 0) {
  throw "debug output does not contain a workload typed archive maintenance timing summary section"
}
...
for ($Index = $SectionIndex + 1; $Index -lt $Lines.Count; $Index++) {
  ...
  if ($Trimmed.StartsWith("workload | action | reason | status write required | base records | pending append | tombstones | resulting records | index before bytes | index after bytes | index byte delta | tombstone before bytes | tombstone after bytes | tombstone byte delta | logical before bytes | logical after bytes | logical byte delta | matched after maintenance | maintenance | agreement")) {
    $HeaderIndex = $Index
    break
  }
}
...
if ($HeaderIndex -lt 0) {
  throw "workload typed archive maintenance timing summary header was not found"
}
...
for ($Index = $HeaderIndex + 1; $Index -lt $Lines.Count; $Index++) {
  ...
  if ($Columns.Count -ne 20) {
    throw "unexpected typed archive maintenance timing summary column count at line $($Index + 1): $($Columns.Count)"
  }
...
if ($Rows.Count -eq 0) {
  throw "workload typed archive maintenance timing summary contained no workload rows"
}
...
$FailedRows = @($Rows | Where-Object { $_[21] -ne "pass" })
$TotalMatchedRecordsAfterMaintenance = 0

foreach ($Row in $Rows) {
  $TotalMatchedRecordsAfterMaintenance += Convert-ToInvariantIntOrZero -Value $Row[19]
}
...
$TombstoneByteDeltaValues = @($Rows | ForEach-Object { $_[15] } | Sort-Object -Unique)
$LogicalArchiveByteDeltaValues = @($Rows | ForEach-Object { $_[18] } | Sort-Object -Unique)

Set-Utf8Text -Path $OutputNotesPath -Text "Typed archive maintenance timing summary notes`r`n"
...
if ($FailedRows.Count -gt 0) {
  ...
  foreach ($FailedRow in $FailedRows) {
    Add-Utf8Text -Path $OutputNotesPath -Text "$($FailedRow[2]): $($FailedRow[21])`r`n"
  }
}

Write-Host "Typed archive maintenance timing summary written:"
