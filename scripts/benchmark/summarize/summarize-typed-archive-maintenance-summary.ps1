...
for ($Index = $SectionIndex + 1; $Index -lt $Lines.Count; $Index++) {
  ...
  if ($Trimmed.StartsWith("workload | action | reason | base records | pending append | tombstones | resulting records | index before bytes | index after bytes | index byte delta | tombstone before bytes | tombstone after bytes | tombstone byte delta | logical before bytes | logical after bytes | logical byte delta | matched after maintenance | maintenance | agreement")) {
    $HeaderIndex = $Index
    break
  }
}
...
$FailedRows = @($Rows | Where-Object { $_[20] -ne "pass" })
$TotalMatchedRecordsAfterMaintenance = 0

foreach ($Row in $Rows) {
  $TotalMatchedRecordsAfterMaintenance += Convert-ToInvariantIntOrZero -Value $Row[18]
}
...
$TombstoneByteDeltaValues = @($Rows | ForEach-Object { $_[14] } | Sort-Object -Unique)
$LogicalArchiveByteDeltaValues = @($Rows | ForEach-Object { $_[17] } | Sort-Object -Unique)
...
if ($FailedRows.Count -gt 0) {
  ...
  foreach ($FailedRow in $FailedRows) {
    Add-Utf8Text -Path $OutputNotesPath -Text "$($FailedRow[2]): $($FailedRow[20])`r`n"
  }
}
