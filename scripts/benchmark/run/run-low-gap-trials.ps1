...
$BenchmarkLowGapTrialsLibrary = Join-Path (Join-Path $ScriptsRoot "lib") "benchmark-low-gap-trials.ps1"

if (!(Test-Path -LiteralPath $BenchmarkLowGapTrialsLibrary)) {
    throw "benchmark low-gap trial helper library was not found: $BenchmarkLowGapTrialsLibrary"
}

. $BenchmarkLowGapTrialsLibrary
if ($MaxDepth -gt 0) {
...
for ($TrialNumber = 1; $TrialNumber -le $Trials; $TrialNumber++) {
    ...
    $AllTrialRows += Read-LowGapTrialRows `
        -TrialNumber $TrialNumber `
        -LowGapCsvPath $TrialLowGapCsvPath `
        -TreeBaselines $TreeBaselines

    $AllWorkloadRows += Read-LowGapWeakestWorkloadRows `
        -TrialNumber $TrialNumber `
        -WorkloadsCsvPath $TrialWorkloadsCsvPath `
        -TreeBaselines $TreeBaselines
}
...
foreach ($BaselineName in $TreeBaselines) {
    $SummaryObjects += New-LowGapTrialSummaryRow `
        -BaselineName $BaselineName `
        -TrialRows $AllTrialRows `
        -PreviousRows $PreviousCsvLoad.Rows `
        -LowGapNoiseThreshold $LowGapNoiseThreshold
}
...
foreach ($BaselineName in $TreeBaselines) {
    $WorkloadSummaryObjects += New-LowGapWeakestWorkloadSummaryRow `
        -BaselineName $BaselineName `
