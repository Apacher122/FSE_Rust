...
fn csv_metadata_can_be_built_from_benchmark_overview() {
    ...
    assert_eq!(metadata.index_scalar_size_bytes, scalar_size_bytes);
    assert_eq!(
        metadata.index_encoded_coordinate_estimated_bytes,
        8 * scalar_size_bytes
    );
    assert_eq!(
        metadata.index_residual_estimated_bytes,
        8 * scalar_size_bytes
    );
    assert_eq!(
        metadata.index_centroid_estimated_bytes,
        6 * scalar_size_bytes
    );
    assert_eq!(
        metadata.index_bounds_estimated_bytes,
        12 * scalar_size_bytes
    );
    assert_eq!(
        metadata.index_structural_metadata_estimated_bytes,
        18 * scalar_size_bytes
    );
    assert_eq!(metadata.index_total_estimated_bytes, 26 * scalar_size_bytes);
    ...
    assert!(metadata.leaf_ownership_cardinality_valid);
...
fn csv_export_with_metadata_includes_metadata_header() {
    ...
    assert!(csv.contains("index_scalar_size_bytes"));
    assert!(csv.contains("index_total_estimated_bytes"));
...
fn csv_export_with_metadata_includes_metadata_values() {
    ...
    assert!(rows[1].starts_with(
        "60,15,6,\"flat_scan, kd_tree\",3,4,8,8,parallel,2,8,7,60,4,8,7.500000,120.000000,15.000000,0.500000,1,8,8,6,12,18,26,1.000000,2.250000,3.250000,4,32,32,24,48,72,104,8,26,18,3.250000,1.000000,2.250000,0.692308,true,true,true,true,true,true,true,true,true,true,true"
    ));
}
...
fn csv_export_with_metadata_writes_summary_file() {
    ...
    assert!(written.contains("index_scalar_size_bytes"));
    assert!(written.contains("index_total_estimated_bytes"));
...
fn workload_csv_export_includes_header() {
    ...
    assert!(csv.contains("reference_average_elapsed_ns"));
...
#[test]
fn workload_csv_export_includes_reference_result_metrics() {
    let fixture = small_benchmark_fixture();
    let registry = BaselineRegistry::new();
    let baseline_kinds = [BaselineKind::FlatScan];
    ...
    assert!(csv.contains("reference_visited_nodes"));
    assert!(csv.contains("reference_retained_leaves"));
    assert!(csv.contains("reference_reconstructed_records"));
    assert!(csv.contains("reference_matched_records"));
    assert!(csv.contains("reference_average_elapsed_ns"));
    assert!(csv.contains("estimated_owned_vs_reference_overhead_ns"));
    assert!(csv.contains("reference_result_speedup_ratio"));
    assert!(csv.contains("reference_stats_match_count_only"));
