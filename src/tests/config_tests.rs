use crate::benchmark::{
    BenchmarkDatasetKind, BenchmarkSuiteConfig, clustered_points_2d, clustered_workload_cases,
...
fn benchmark_suite_config_build_config_matches_values() {
    let config = BenchmarkSuiteConfig::new(BenchmarkDatasetKind::SmallClustered2D, 16, 12, 5);
...
fn benchmark_suite_config_returns_small_dataset() {
    let config = BenchmarkSuiteConfig::new(BenchmarkDatasetKind::SmallClustered2D, 8, 8, 10);
...
fn benchmark_suite_config_returns_large_dataset() {
    let config = BenchmarkSuiteConfig::new(BenchmarkDatasetKind::LargeClustered2D, 8, 8, 10);
