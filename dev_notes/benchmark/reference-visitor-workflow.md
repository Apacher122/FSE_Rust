...
## Small dataset evidence: c160-reference-visitor-materialization-small
...
## Large dataset evidence: c161-reference-visitor-materialization-large
...
large_cluster_range_000:
  reference-result: 264ns
  reference-visitor: 234ns
  visitor advantage: 30ns
  reference/visitor ratio: 1.13x

large_cluster_range_001:
  reference-result: 240ns
  reference-visitor: 258ns
  visitor result: 18ns slower

large_cluster_range_002:
  reference-result: 232ns
  reference-visitor: 209ns
  visitor advantage: 23ns
  reference/visitor ratio: 1.11x

large_cluster_range_003:
  reference-result: 244ns
  reference-visitor: 265ns
  visitor result: 21ns slower

large_cluster_range_004:
  reference-result: 263ns
  reference-visitor: 253ns
  visitor advantage: 10ns
  reference/visitor ratio: 1.04x

large_cluster_range_005:
  reference-result: 241ns
  reference-visitor: 238ns
  visitor advantage: 3ns
  reference/visitor ratio: 1.01x

large_cluster_range_006:
  reference-result: 229ns
  reference-visitor: 204ns
  visitor advantage: 25ns
  reference/visitor ratio: 1.12x

large_cluster_range_007:
  reference-result: 246ns
  reference-visitor: 238ns
...
large_cluster_range_008:
  ...
  visitor advantage: 29ns
  reference/visitor ratio: 1.14x

large_cluster_range_009:
  reference-result: 264ns
  reference-visitor: 246ns
  visitor advantage: 18ns
  reference/visitor ratio: 1.07x

large_empty_far_range:
  reference-result: 22ns
  reference-visitor: 31ns
  visitor result: 9ns slower

large_full_dataset_range:
  matched records: 10000
  reference-result: 11.586us
  reference-visitor: 4.082us
  visitor advantage: 7.504us
  reference/visitor ratio: 2.84x

large_cross_cluster_boundary:
  matched records: 71
  reference-result: 372ns
  reference-visitor: 348ns
  visitor advantage: 24ns
  reference/visitor ratio: 1.07x
```
...
```text
reference visitor is faster than reference-result on 10 of 13 large workloads
small slower rows are in the 9ns to 21ns range
...
The target debug section uses an independent timing probe and reported a noisier local result:

```text
reference-result average elapsed: 362ns
reference-visitor average elapsed: 396ns
count-only average elapsed: 316ns
reference visitor stats match reference: true
all matched records agree: true
```
...
```text
large_cross_cluster_boundary is near the noise boundary for reference-result versus visitor timing
...
The c160 and c161 runs are useful output-contract evidence, not broad FSE performance proof.
